...
\begin{frame}
    ...
    %     \centering
    %     \includegraphics[width=0.7\textwidth]{imagenes/Caso2.1c.png}
    %     %\caption{}
    % \end{wrapfigure}
%    \hspace*{1cm}
        \includegraphics[width=0.8\textwidth]{imagenes/Caso2.1c.png}
\end{frame}
...
\textbf{Non-winding polygon: O(n) algorithm}\\
    \vspace{0.5cm}
    El problema de la visibilidad de puntos en polígonos non-windings se centra en determinar qué puntos dentro de dichos polígonos son visibles desde un punto de referencia específico.
\end{frame}
...
\begin{frame}{Caso 2b.ii}
    ...
    \begin{columns}
    \begin{column}{0.45\textwidth}
    $u$ no es un vértice de $P$. Sea $w$ el vértice que se encuentra justo debajo de $u$ en el stack. Por lo que, $uw$ es un \textit{constructed edge}\\
    \vspace{0.5cm}
    \end{column}
    \begin{column}{0.45\textwidth}  %%<--- here
    \vspace{-3cm}
